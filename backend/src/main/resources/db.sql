...
CREATE TABLE `compliance_policy` (
  ...
  `company_id` BIGINT COMMENT '公司ID',
  `policy_type` VARCHAR(32) COMMENT '策略类型',
  `priority` INT DEFAULT 50 COMMENT '优先级，越小越高',
  `name` VARCHAR(100) NOT NULL COMMENT '策略名称',
  `rule_content` TEXT COMMENT '规则内容（JSON/IF-THEN）',
  `scope` VARCHAR(50) COMMENT '生效范围（全局/指定资产/模型）',
  `scope_departments` VARCHAR(500) COMMENT '适用部门',
  `scope_user_groups` VARCHAR(500) COMMENT '适用用户组',
  `scope_data_types` VARCHAR(500) COMMENT '适用数据类型',
  `last_modifier` VARCHAR(128) COMMENT '最近修改人',
